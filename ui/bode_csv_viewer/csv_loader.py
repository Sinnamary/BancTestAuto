...
class BodeCsvFileLoader:
    ...
    def _row_to_point(self, row: List[str], col_map: BodeCsvColumnMap) -> Optional[BodeCsvPoint]:
        try:
            i_f = col_map.get("f_hz")
            i_us = col_map.get("us_v")
            i_lin = col_map.get("gain_linear")
            i_db = col_map.get("gain_db")
            f_hz = float(row[i_f].replace(",", "."))
            us_v = float(row[i_us].replace(",", ".")) if i_us < len(row) else 0.0
            g_db = float(row[i_db].replace(",", "."))
            g_lin = float(row[i_lin].replace(",", ".")) if i_lin < len(row) else self._db_to_linear(g_db)
            return BodeCsvPoint(f_hz=f_hz, us_v=us_v, gain_linear=g_lin, gain_db=g_db)
