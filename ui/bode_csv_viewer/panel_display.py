...
def build_display_panel(parent, on_options_changed) -> QGroupBox:
    ...
    parent._show_gain_cb = QCheckBox("Gain")
    parent._show_gain_cb.setChecked(True)
    parent._show_gain_cb.setToolTip("Afficher la courbe de gain (axe gauche)")
    parent._show_gain_cb.toggled.connect(on_options_changed)
    layout.addWidget(parent._show_gain_cb)
    parent._show_phase_cb = QCheckBox("Phase")
    parent._show_phase_cb.setChecked(True)
    parent._show_phase_cb.setToolTip("Afficher la courbe de phase (axe droit, °) — si le CSV contient Phase_deg")
    parent._show_phase_cb.toggled.connect(on_options_changed)
    layout.addWidget(parent._show_phase_cb)
    layout.addStretch()
    return gb
